...
import { useBorrow as useBorrowEvm } from '../_evm/lending/borrow';
import { useBorrow as useBorrowFpass } from '../_evm/lending/borrow-substrate';
...
export const useLendingBorrow = ({ token, enabled, debug: _debug }: Props) => {
  const { isFpass } = useNetwork();

  const resEvm = useBorrowEvm({ token, enabled });
  const resFpass = useBorrowFpass({ token, enabled });
