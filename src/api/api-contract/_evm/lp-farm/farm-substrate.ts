...
import { useFarmNetworkFeeErrorStore } from '~/states/contexts/network-fee-error/network-fee-error';
...
export const useFarmSubstrate = ({ poolId, farmAmount, enabled }: Props) => {
  const { setError } = useFarmNetworkFeeErrorStore();
  ...
  const estimateFee = async () => {
    ...
    try {
      ...
      const encodedData =
        isFpass && !!walletAddress && !!signer && !!poolId && !!farmAmount
          ? encodeFunctionData({
              abi: LP_FARM_ABI,
...
export const useFarmPrepare = ({ poolId, farmAmount, enabled }: Props) => {
  const { fpass } = useConnectedWallet();
  ...
  const {
    ...
    chainId,
    enabled:
      enabled && isEvm && isFpass && !!walletAddress && !!farmAmount && !!poolId && !!chainId,
  });
