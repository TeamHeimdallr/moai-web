...
interface Props {
  mtoken?: IToken & { balance: number; amount: number };
  enabled?: boolean;
}
export const useSupply = ({ mtoken, enabled }: Props) => {
  ...
  const { isLoading: prepareLoading, config } = usePrepareContractWrite({
    address: (mtoken?.address || '') as Address,
    ...
    chainId,
    args: [mtoken?.amount],
    enabled: enabled && isConnected && isEvm && !isFpass && !!walletAddress && !!mtoken,
  });
