...
import { useSwap } from '~/moai-xrp-evm/api/api-contract/swap/swap';
...
import { SwapKind } from '~/moai-xrp-evm/types/contracts';
...
export const PopupSwap = () => {
  ...
  const { data, blockTimestamp, isLoading, isSuccess, error, swap } = useSwap({
    singleSwap: [
      ...
      TOKEN_ADDRESS[toToken] === TOKEN_ADDRESS.XRP ? TOKEN_ADDRESS.ZERO : TOKEN_ADDRESS[toToken],
      parseUnits(`${fromValue ?? 0}`, TOKEN_DECIAML),
      '0x0',
    ],
    fundManagement: [address ?? '0x0', false, address ?? '0x0', false],
    enabled: allowance,
  });
