...
import { LendingSupplyInputGroup } from './components/supply-input-group';

export const LendingSupply = () => {
  useGAPage();
  useForceNetwork({
  ...
  const { ref } = useGAInView({ name: 'lending-supply' });
        ...
        {networkFull === selectedNetwork && market && market?.symbol && (
          <ContentWrapper>
            <Header>
              ...
              <Title>{t('supply-token', { token: market?.underlyingSymbol })}</Title>
            </Header>

            <SupplyWrapper>
              <LendingSupplyInputGroup />
            </SupplyWrapper>
          </ContentWrapper>
        )}
      </InnerWrapper>
